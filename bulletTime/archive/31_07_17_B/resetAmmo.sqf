    ...
    _magBaseName = _magBaseName select 0;
        ...
        { 
            if ((_x select 0) isEqualTo _unitName) then
            {
				...
				if ((_x select 0) isEqualTo (name player)) then { hint format ["%1, %2, %3", _unitName, _previousMag, _magNameAmmoList]; };
                
            }  
        } forEach ammoList;
            
        hint format ["%1", _magNameAmmoList];
